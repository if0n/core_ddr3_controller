...
`endif
    ...
    reg                         auto_self_refresh           ;   // auto self refresh
    reg                         self_refresh_temperature    ;   // self refresh temperature range
    reg     [1:0]               mpr_select                  ;
    reg                         mpr_en                      ;
    reg                         odts_readout                ;
    ...
    initial begin
        cmd_string[LOAD_MODE] = "Load Mode";    /// 0
        cmd_string[REFRESH  ] = "Refresh  ";    /// 1
        cmd_string[PRECHARGE] = "Precharge";    /// 2
        cmd_string[ACTIVATE ] = "Activate ";    /// 3
        cmd_string[WRITE    ] = "Write    ";    /// 4
        cmd_string[READ     ] = "Read     ";    /// 5
        cmd_string[ZQ       ] = "ZQ       ";    /// 6
        cmd_string[NOP      ] = "No Op    ";    /// 7
        cmd_string[PWR_DOWN ] = "Pwr Down ";    /// 8
        cmd_string[SELF_REF ] = "Self Ref ";    /// 9
    end
    ...
    reg     [5:0]               cmd_tran_index, cmd_tran_i; /// noused
        ...
        `ifdef mem_init
            ...
            while (! $feof(in)) begin
                ...
                if (fio_status != -1) begin // Check for blank line or EOF
                    ...
                    memory_write_task (bank, row, col, data);
                    ...
                    memory_read_task(bank, row, col, data);
...
`endif
    ...
    assign TZQCS   = max_func( 64, ceil_func( 80000/tck_avg));
    assign TZQINIT = max_func(512, ceil_func(640000/tck_avg));
    assign TZQOPER = max_func(256, ceil_func(320000/tck_avg));
    ...
    function integer get_rtt_wr_func;    /// 根据写操作时的 ODT 设置（rtt），计算实际的终端电阻值
        input [1:0] rtt;
        begin
            get_rtt_wr_func = RZQ/{rtt[0], rtt[1], 1'b0};
        end
    endfunction

    function integer get_rtt_nom_func;   /// 根据正常（空闲）状态下的 ODT 设置（rtt），计算实际的终端电阻值
        input [2:0] rtt;
        begin
            case (rtt)
                1: get_rtt_nom_func = RZQ/4;
                2: get_rtt_nom_func = RZQ/2;
                3: get_rtt_nom_func = RZQ/6;
                4: get_rtt_nom_func = RZQ/12;
                5: get_rtt_nom_func = RZQ/8;
                default : get_rtt_nom_func = 0;
            endcase
        end
    endfunction

    // calculate the absolute value of a real number
    function real abs_value_func;
        input arg;
        real arg;
        begin
            if (arg < 0.0)
                abs_value_func = -1.0 * arg;
            else
                abs_value_func = arg;
        end
    endfunction

    function integer ceil_func;
        input number;
        real number;
        ...
        if (number > $rtoi(number))
            ceil_func = $rtoi(number) + 1;
        else
            ceil_func = number;
    endfunction

    function integer floor_func;
        input number;
        real number;
        ...
        // to an equivalent real value, and the expression shall be interpreted as a comparison between two real values.
        if (number < $rtoi(number))
            floor_func = $rtoi(number) - 1;
        else
            floor_func = number;
    endfunction

    function int max_func( input int a, b );
        max_func = (a < b) ? b : a;
    endfunction

    function int min_func( input int a, b );
        min_func = (a > b) ? b : a;
    endfunction

`ifdef MAX_MEM

    function integer open_bank_file_func( input integer bank );
        integer fd;
        reg [2048:1] filename;
        begin
            ...
            if (fd == 0) begin
                ...
                open_bank_file_func = fd;
            end

        end
    endfunction

    function [RFF_BITS:1] read_from_file_func(
        input integer fd,
        input integer index
        );
        integer code;
        ...
        begin
            ...
            */
            ...
            read_from_file_func = read_value;
        end
    endfunction

    task write_to_file_task(
        input integer fd,
        input integer index,
...
`ifdef MAX_MEM
            addr = {row, col}/BL_MAX;
            write_to_file_task( memfd[bank], addr, data );
`else
...
`ifdef MAX_MEM
            addr = {row, col}/BL_MAX;
            data = read_from_file_func( memfd[bank], addr );
`else
            // chop off the lowest address bits
            addr = {bank, row, col}/BL_MAX;
            if (get_index(addr)) begin
...
`endif
    ...
    task set_latency_task;   /// task set WL/RL, set al and dll_en first
        begin
            if (al == 0)    /// 模式寄存器配置al: MR1
                additive_latency = 0;
            else
                additive_latency = cas_latency - al;    /// 加性延迟值

            if (dll_en) /// dll使能，表示DLL是否启用
                read_latency = cas_latency + additive_latency;  /// 读延迟 RL = CL + AL - (dll_en ? 0 : 1)
            else
                read_latency = cas_latency + additive_latency - 1;

            write_latency = cas_write_latency + additive_latency;   /// 写延迟 WL = CWL + AL
        end
    endtask

    // this task will erase the contents of 0 or more banks
    task erase_banks_task;
...
`ifdef MAX_MEM
            for (bank = 0; bank < `BANKS; bank = bank + 1)
                if (banks[bank] === 1'b1) begin
	                $fclose(memfd[bank]);
                    memfd[bank] = open_bank_file_func(bank);
	            end
`else
            memory_index = 0;
            i = 0;
...
`endif
    ...
    task initialize_task;    /// noused
        ...
        begin
            ...
            cmd_task(prev_cke, 1,       NOP, 'bx, 'bx               );  // 向内存设备发送一个 NOP 命令，通常用于占位或满足时序要求
            cmd_task(prev_cke, 1,        ZQ, 'bx, 'h400             );  // ZQCL，长时间的ZQ校准，提供最精确的片上终端电阻（ODT）校准
            cmd_task(prev_cke, 1, LOAD_MODE,   3, mode_reg3         );  // MR3
            cmd_task(prev_cke, 1, LOAD_MODE,   2, mode_reg2         );  // MR2
            cmd_task(prev_cke, 1, LOAD_MODE,   1, mode_reg1         );  // MR1
            cmd_task(prev_cke, 1, LOAD_MODE,   0, mode_reg0 | 'h100 );  // MR0 | DLL Reset
            cmd_task(prev_cke, 0,       NOP, 'bx, 'bx);                 // 发送 NOP 命令，并禁用时钟，使内存设备进入特定的空闲或省电状态
        end
    endtask
    ...
    task chk_err_task;
        ...
        begin
            // $display ("%m: at time %t: truebl4 = %d, relationship = %d, fromcmd = %s, cmd = %s", $time, truebl4, relationship, cmd_string[fromcmd], cmd_string[cmd]);
            casex ({truebl4, relationship, fromcmd, cmd})
                // load mode
                {1'bx, DIFF_BANK , LOAD_MODE, LOAD_MODE} : begin if (ck_cntr - ck_load_mode < TMRD)
                                                                    $display ("%m: at time %t ERROR:  tMRD violation during %s", $time, cmd_string[cmd]);                         end
                ...
                {1'bx, DIFF_BANK , LOAD_MODE, SELF_REF } : begin if (($time - tm_load_mode < TMOD-TJIT_PER) || (ck_cntr - ck_load_mode < TMOD_TCK))
                                                                    $display ("%m: at time %t ERROR:  tMOD violation during %s", $time, cmd_string[cmd]);                         end
                ...
                {1'bx, DIFF_BANK , REFRESH  , SELF_REF } : begin if ($time - tm_refresh < TRFC_MIN)
                                                                    $display ("%m: at time %t ERROR:  tRFC violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , REFRESH  , PWR_DOWN } : begin if (ck_cntr - ck_refresh < TREFPDEN)
                                                                    $display ("%m: at time %t ERROR:  tREFPDEN violation during %s", $time, cmd_string[cmd]);                     end

                // precharge
                {1'bx, SAME_BANK , PRECHARGE, ACTIVATE } : begin if ($time - tm_bank_precharge[bank] < TRP-TJIT_PER)
                                                                    $display ("%m: at time %t ERROR:   tRP violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'bx, DIFF_BANK , PRECHARGE, LOAD_MODE} ,
                {1'bx, DIFF_BANK , PRECHARGE, REFRESH  } ,
                {1'bx, DIFF_BANK , PRECHARGE, SELF_REF } : begin if ($time - tm_precharge < TRP-TJIT_PER)
                                                                    $display ("%m: at time %t ERROR:   tRP violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , PRECHARGE, ZQ       } : begin if ($time - tm_precharge < TRP)
                                                                    $display ("%m: at time %t ERROR:   tRP violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , PRECHARGE, PWR_DOWN } :          ; //tPREPDEN = 1 tCK, can be concurrent with auto precharge

                // activate
                {1'bx, SAME_BANK , ACTIVATE , PRECHARGE} : begin if ($time - tm_bank_activate[bank] > TRAS_MAX)
                                                                 ...
                                                                 if ($time - tm_bank_activate[bank] < TRAS_MIN-TJIT_PER)
                                                                    $display ("%m: at time %t ERROR:  tRAS minimum violation during %s to bank %d", $time, cmd_string[cmd], bank);end
                {1'bx, SAME_BANK , ACTIVATE , ACTIVATE } : begin if ($time - tm_bank_activate[bank] < TRC-TJIT_PER)
                                                                    $display ("%m: at time %t ERROR:   tRC violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'bx, SAME_BANK , ACTIVATE , WRITE    } ,
                {1'bx, SAME_BANK , ACTIVATE , READ     } :          ; // tRCD is checked outside this task
                {1'b0, DIFF_BANK , ACTIVATE , ACTIVATE } : begin if (($time - tm_activate < TRRD) || (ck_cntr - ck_activate < TRRD_TCK))
                                                                    $display ("%m: at time %t ERROR:  tRRD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_BANK , ACTIVATE , ACTIVATE } : begin if (($time - tm_group_activate[bank[1]] < TRRD) || (ck_cntr - ck_group_activate[bank[1]] < TRRD_TCK))
                                                                    $display ("%m: at time %t ERROR:  tRRD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_GROUP, ACTIVATE , ACTIVATE } : begin if (($time - tm_activate < TRRD_DG) || (ck_cntr - ck_activate < TRRD_DG_TCK))
                                                                    $display ("%m: at time %t ERROR:  tRRD_DG violation during %s to bank %d", $time, cmd_string[cmd], bank);     end
                {1'bx, DIFF_BANK , ACTIVATE , REFRESH  } : begin if ($time - tm_activate < TRC-TJIT_PER)
                                                                    $display ("%m: at time %t ERROR:   tRC violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , ACTIVATE , PWR_DOWN } : begin if (ck_cntr - ck_activate < TACTPDEN)
                                                                    $display ("%m: at time %t ERROR:  tACTPDEN violation during %s", $time, cmd_string[cmd]);                     end

                // write
                {1'bx, SAME_BANK , WRITE    , PRECHARGE} : begin if (($time - tm_bank_write_end[bank] < TWR-TJIT_PER) || (ck_cntr - ck_bank_write[bank] <= write_latency + burst_length/2))
                                                                    $display ("%m: at time %t ERROR:   tWR violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b0, DIFF_BANK , WRITE    , WRITE    } : begin if (ck_cntr - ck_write < TCCD)
                                                                    $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_BANK , WRITE    , WRITE    } : begin if (ck_cntr - ck_group_write[bank[1]] < TCCD)
                                                                    $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b0, DIFF_BANK , WRITE    , READ     } : begin if (ck_cntr - ck_write < write_latency + burst_length/2 + TWTR_TCK - additive_latency)
                                                                    $display ("%m: at time %t ERROR:  tWTR violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_BANK , WRITE    , READ     } : begin if (ck_cntr - ck_group_write[bank[1]] < write_latency + burst_length/2 + TWTR_TCK - additive_latency)
                                                                    $display ("%m: at time %t ERROR:  tWTR violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_GROUP, WRITE    , WRITE    } : begin if (ck_cntr - ck_write < TCCD_DG)
                                                                    $display ("%m: at time %t ERROR:  tCCD_DG violation during %s to bank %d", $time, cmd_string[cmd], bank);     end
                {1'b1, DIFF_GROUP, WRITE    , READ     } : begin if (ck_cntr - ck_write < write_latency + burst_length/2 + TWTR_DG_TCK - additive_latency)
                                                                    $display ("%m: at time %t ERROR:  tWTR_DG violation during %s to bank %d", $time, cmd_string[cmd], bank);     end
                {1'bx, DIFF_BANK , WRITE    , PWR_DOWN } : begin if (($time - tm_write_end < TWR-TJIT_PER) || (ck_cntr - ck_write < write_latency + burst_length/2))
                                                                    $display ("%m: at time %t ERROR:  tWRPDEN violation during %s", $time, cmd_string[cmd]);                      end

                // read
                {1'bx, SAME_BANK , READ     , PRECHARGE} : begin if (($time - tm_bank_read_end[bank] < TRTP-TJIT_PER) || (ck_cntr - ck_bank_read[bank] < additive_latency + TRTP_TCK))
                                                                    $display ("%m: at time %t ERROR:  tRTP violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b0, DIFF_BANK , READ     , WRITE    } :          ; // tRTW is checked outside this task
                {1'b1, DIFF_BANK , READ     , WRITE    } :          ; // tRTW is checked outside this task
                {1'b0, DIFF_BANK , READ     , READ     } : begin if (ck_cntr - ck_read < TCCD)
                                                                    $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_BANK , READ     , READ     } : begin if (ck_cntr - ck_group_read[bank[1]] < TCCD)
                                                                    $display ("%m: at time %t ERROR:  tCCD violation during %s to bank %d", $time, cmd_string[cmd], bank);        end
                {1'b1, DIFF_GROUP, READ     , WRITE    } :          ; // tRTW is checked outside this task
                {1'b1, DIFF_GROUP, READ     , READ     } : begin if (ck_cntr - ck_read < TCCD_DG)
                                                                    $display ("%m: at time %t ERROR:  tCCD_DG violation during %s to bank %d", $time, cmd_string[cmd], bank);     end
                {1'bx, DIFF_BANK , READ     , PWR_DOWN } : begin if (ck_cntr - ck_read < read_latency + 5)
                                                                    $display ("%m: at time %t ERROR:  tRDPDEN violation during %s", $time, cmd_string[cmd]);                      end
                ...
                {1'bx, DIFF_BANK , ZQ       , SELF_REF } : begin if (ck_cntr - ck_zqinit < TZQINIT)
                                                                 ...
                                                                 if (ck_cntr - ck_zqcs < TZQCS)
                                                                    $display ("%m: at time %t ERROR:  tZQCS violation during %s", $time, cmd_string[cmd]);                        end
                ...
                {1'bx, DIFF_BANK , PWR_DOWN , ZQ       } : begin if (($time - tm_power_down < TXP) || (ck_cntr - ck_power_down < TXP_TCK))
                                                                    $display ("%m: at time %t ERROR:   tXP violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , PWR_DOWN , READ     } : begin if (($time - tm_power_down < TXP) || (ck_cntr - ck_power_down < TXP_TCK))
                                                                    ...
                                                                    $display ("%m: at time %t ERROR:  tXPDLL violation during %s", $time, cmd_string[cmd]);                       end
                {1'bx, DIFF_BANK , PWR_DOWN , PWR_DOWN } ,
                {1'bx, DIFF_BANK , PWR_DOWN , SELF_REF } : begin if (($time - tm_power_down < TXP) || (ck_cntr - ck_power_down < TXP_TCK))
                                                                    $display ("%m: at time %t ERROR:   tXP violation during %s", $time, cmd_string[cmd]);
                                                                 ...
                                                                 if (($time - tm_cke_cmd < TCKE) || (ck_cntr - ck_cke_cmd < TCKE_TCK))
                                                                    $display ("%m: at time %t ERROR:  tCKE violation on CKE", $time);                                             end
                ...
                {1'bx, DIFF_BANK , SELF_REF , ZQ       } : begin if (($time - tm_self_refresh < TXS) || (ck_cntr - ck_self_refresh < TXS_TCK))
                                                                    $display ("%m: at time %t ERROR:   tXS violation during %s", $time, cmd_string[cmd]);                         end
                {1'bx, DIFF_BANK , SELF_REF , READ     } : begin if (ck_cntr - ck_self_refresh < TXSDLL)
                                                                    $display ("%m: at time %t ERROR:  tXSDLL violation during %s", $time, cmd_string[cmd]);                       end
                {1'bx, DIFF_BANK , SELF_REF , PWR_DOWN } ,
                {1'bx, DIFF_BANK , SELF_REF , SELF_REF } : begin if (($time - tm_self_refresh < TXS) || (ck_cntr - ck_self_refresh < TXS_TCK))
        ...
        begin
            ...
            if (cke) begin  /// cke 1
                if ((cmd < NOP) && (cmd != PRECHARGE)) begin
                    ...
                    /// check err from fromcmd(prev_cmd) of all other type to cmd
                    for (j=0; j<=SELF_REF; j=j+1) begin
                        chk_err_task(SAME_BANK , bank, j, cmd);
                        chk_err_task(DIFF_BANK , bank, j, cmd);
                        chk_err_task(DIFF_GROUP, bank, j, cmd);
                    end
                end
                ...
                case (cmd)
                    ...
                    LOAD_MODE :
                        begin
                            ...
                            if (|active_bank) begin
                                ...
                                if (bank>>2) $display ("%m: at time %t ERROR: %s %d Illegal value.  Reserved bank bits must be programmed to zero", $time, cmd_string[cmd], bank);
                                ...
                                case (bank)
                                    ...
                                    0 :
                                        begin
                                            ...
                                            if (addr[1:0] == 2'b00) begin
                                                burst_length = 8;   /// 8(Fixed)
                                                blotf = 0;
                                                truebl4 = 0;
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Burst Length = %d", $time, cmd_string[cmd], bank, burst_length);
                                            end else if (addr[1:0] == 2'b01) begin
                                                burst_length = 8;   /// BC4 or 8(on the fly)
                                                blotf = 1;
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Burst Length = Select via A12", $time, cmd_string[cmd], bank);
                                            end else if (addr[1:0] == 2'b10) begin
                                                burst_length = 4;   /// BC4(Fixed)
                                                blotf = 0;
                                            ...
                                            set_latency_task;    /// task set WL/RL, set al and dll_en first
                                    ...
                                    1 :
                                        begin
                                            ...
                                            if (odt_rtt_nom == 3'b000) begin
                                                ...
                                                odt_en = 0;
                                            end else if ((odt_rtt_nom < 4) || ((!addr[7] || (addr[7] && addr[12])) && (odt_rtt_nom < 6))) begin /// 7:Write leveling enable, 12:Qoff
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d ODT Rtt = %d Ohm", $time, cmd_string[cmd], bank, get_rtt_nom_func(odt_rtt_nom));
                                                odt_en = 1;
                                            end else begin
                                    ...
                                    2 :
                                        begin
			    		    		        if (feature_pasr) begin /// parameter set
                                            ...
                                            auto_self_refresh = addr[6];
                                            if (!auto_self_refresh) begin
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Auto Self Refresh = Disabled", $time, cmd_string[cmd], bank);
                                            end else if (auto_self_refresh) begin
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Auto Self Refresh = Enabled", $time, cmd_string[cmd], bank);
                                                if (check_strict_mrbits) $display ("%m: at time %t WARNING: %s %d Auto Self Refresh is not modeled", $time, cmd_string[cmd], bank);
                                            end else begin
                                                $display ("%m: at time %t ERROR: %s %d Illegal Auto Self Refresh = %d", $time, cmd_string[cmd], bank, auto_self_refresh);
                                            end

                                            // Self Refresh Temperature
                                            self_refresh_temperature = addr[7];
                                            if (!self_refresh_temperature) begin
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Self Refresh Temperature = Normal", $time, cmd_string[cmd], bank);
                                            end else if (self_refresh_temperature) begin
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Self Refresh Temperature = Extended", $time, cmd_string[cmd], bank);
                                                if (check_strict_mrbits) $display ("%m: at time %t WARNING: %s %d Self Refresh Temperature is not modeled", $time, cmd_string[cmd], bank);
                                            end else begin
                                                $display ("%m: at time %t ERROR: %s %d Illegal Self Refresh Temperature = %d", $time, cmd_string[cmd], bank, self_refresh_temperature);
                                            end

                                            if (auto_self_refresh && self_refresh_temperature)
                                            ...
                                            if (odt_rtt_wr == 2'b00) begin
                                                ...
                                                dyn_odt_en = 0;
                                            end else if ((odt_rtt_wr > 0) && (odt_rtt_wr < 3)) begin    /// 01, 10
                                                if (DEBUG) $display ("%m: at time %t INFO: %s %d Dynamic ODT Rtt = %d Ohm", $time, cmd_string[cmd], bank, get_rtt_wr_func(odt_rtt_wr));
                                                dyn_odt_en = 1;
                                            end else begin  /// 11
                                ...
                                /// update mode_reg record and mode_reg set status of all bank
                                ...
                                /// update time and clk cnt of LOAD_MODE cmd
                                tm_load_mode <= $time;
                    ...
                    REFRESH :
                        begin
                            if (mpr_en) begin
                                ...
                                er_trfc_max = 0;
                                ref_cntr = ref_cntr + 1;
                                /// update time and clk cnt of REFRESH cmd
                                tm_refresh <= $time;
                                ck_refresh <= ck_cntr;
                            end
                        end
                    ...
                    PRECHARGE :
                        begin
                            ...
                            if (|active_bank) begin
                                    ...
                                    for (i=0; i<`BANKS; i=i+1) begin
                                        if (active_bank[i]) begin
                                            if (addr[AP] || (i == bank)) begin
                                                for (j=0; j<=SELF_REF; j=j+1) begin
                                                    chk_err_task(SAME_BANK, i, j, cmd);
                                                    chk_err_task(DIFF_BANK, i, j, cmd);
                                                end
                                                ...
                                                if (auto_precharge_bank[i]) begin
                                                    ...
                                                    active_bank[i] = 1'b0;
                                                    /// update time/bank time/clk cnt of PRECHARGE cmd
                                ...
                                chk_err_task(DIFF_BANK, 0, REFRESH, PRECHARGE);
                            end
                        end
                    ///
                    ACTIVATE :
                        begin
                            ...
                            if (mpr_en) begin
                                ...
                                // update active status and active row addr record of all banks
                                active_bank[bank] = 1'b1;
                                active_row[bank] = addr;
                                // update time/bank time/group time/cnt/group cnt of ACTIVATE cmd
                    ...
                    WRITE :
                        begin
                            if ((!rd_bc && blotf) || (burst_length == 4)) begin // (not chop && on the fly) || (BL=4)
                                if (truebl4) begin
...
`endif
                                ...
                                if ((!addr[BC] && blotf) || (burst_length == 4)) begin // BL=4
                                    col = col & -4; // 将 col 地址的 最低两位 清零，从而确保 col 是4的倍数
                                end else begin // BL=8
                                    col = col & -8; // 将 col 地址的 最低三位 清零，从而确保 col 是8的倍数
                                end

                                if (DEBUG) $display ("%m: at time %t INFO: %s bank %d col %h, auto precharge %d", $time, cmd_string[cmd], bank, col, addr[AP]);

                                wr_pipeline[2*write_latency + 1]  = 1;                  // 记录写操作状态
                                ba_pipeline[2*write_latency + 1]  = bank;               // 记录访问的bank
                                row_pipeline[2*write_latency + 1] = active_row[bank];   // 记录访问的row
                                col_pipeline[2*write_latency + 1] = col;                // 记录访问的col
                                ...
                                // update cnt/bank cnt of WRITE cmd
...
`endif
                                ...
                                // set read burst chop
                                rd_bc = addr[BC];
                                // update cnt/bank cnt of READ cmd
                        ...
                        begin
                            if (in_power_down) begin
                                ...
                                if ((active_bank == 0) && low_power) begin // precharge power down with dll off
                                    ...
                                    // updata time/cnt of slow exit power down
                                    tm_slow_exit_pd <= $time;
                                    ck_slow_exit_pd <= ck_cntr;
                                end
                                // update time/cnt of power down exit
                                tm_power_down <= $time;
                            ...
                            if (in_self_refresh) begin
                                ...
                                // update time/cnt of self refresh exit
                                in_self_refresh = 0;
                                ck_dll_reset <= ck_cntr;    /// dll reset cnt relate to self refresh exit
            ...
            else if (prev_cke) begin    /// cke 1->0 transfer edge
                ...
                case (cmd)
                    ...
                    REFRESH :
                        begin
                            ...
                            for (j=0; j<=SELF_REF; j=j+1) begin
                                chk_err_task(DIFF_BANK, bank, j, SELF_REF);
                            end
                            ...
                            if (mpr_en) begin
			    		        ...
			    		        if (feature_pasr)
                                    ...
                                    case (pasr)
                                        ...
                                        3'b001 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 4-7 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'hF0); end
                                        3'b010 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 2-7 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'hFC); end
                                        3'b011 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 1-7 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'hFE); end
                                        3'b100 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 0-1 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'h03); end
                                        3'b101 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 0-3 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'h0F); end
                                        3'b110 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 0-5 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'h3F); end
                                        3'b111 : begin if (DEBUG) $display("%m: at time %t INFO: Banks 0-6 will be lost due to Partial Array Self Refresh", $time); erase_banks_task(8'h7F); end
			    		            endcase
                                in_self_refresh = 1;
                                dll_locked = 0;
                            end
                    ...
                    NOP :
                        begin
                            ...
                            for (j=0; j<=SELF_REF; j=j+1) begin
                                chk_err_task(DIFF_BANK, bank, j, PWR_DOWN);
                            end
            ...
            else if (in_self_refresh || in_power_down) begin    /// cke 0
    ...
    task data_task;
        ...
        begin
            ...
            if (diff_ck) begin
                ...
                for (i=0; i<64; i=i+1) begin
                    if (dll_locked && dq_in_valid) begin
                        tm_tdqss = abs_value_func(1.0*tm_ck_pos - tm_dqss_pos[i]);
            ...
            if (wr_pipeline[0] || rd_pipeline[0]) begin
                ...
                col = col_pipeline[0];
                burst_cntr = 0;
                memory_read_task(bank, row, col, memory_data);
            end
            ...
            if (dq_in_valid) begin // write data
                ...
                if (burst_cntr%BL_MIN == 0) begin
                    memory_write_task(bank, row, col, memory_data);
                end
            end
...
`ifdef MAX_MEM
    ...
    //	          $fclose(memfd[banki]);
    //            memfd[banki] = open_bank_file_func(banki);
    //    end
`else
    ...
    always @(negedge rst_n_in or posedge diff_ck or negedge diff_ck) begin : main
        ...
        if (!rst_n_in) begin
            ...
            if (in_self_refresh || (in_power_down && (active_bank == 0))) begin
                ...
                if (dll_locked && (abs_value_func(tjit_per_rtime) > TJIT_PER)) begin
            ...
            if (diff_ck) begin
                ...
                if (dll_locked) begin
                    if (mr_chk == 0) begin
                        ...
                        if (ceil_func(write_recovery*tck_avg) < TWR)
                ...
                if (!in_self_refresh && tm_ck_pos ) begin
                    ...
                    if (dll_locked && check_strict_timing) begin
                        ...
                        for (i=0; i<12; i=i+1) begin
                            ...
                            terr_nper_rtime = abs_value_func(terr_nper_rtime);
                            case (i)
                                0 : ;
                        ...
                        if (abs_value_func(tjit_per_rtime) - TJIT_PER >= 1.0) 
                            $display ("%m: at time %t ERROR: tJIT(per) violation by %f ps.", $time, abs_value_func(tjit_per_rtime) - TJIT_PER);
                        if (abs_value_func(tjit_cc_time) - TJIT_CC >= 1.0) 
                            $display ("%m: at time %t ERROR: tJIT(cc) violation by %f ps.", $time, abs_value_func(tjit_cc_time) - TJIT_CC);
            ...
            if (odt_en || dyn_odt_en) begin
                ...
                if (!in_self_refresh && diff_ck) begin
                    ...
                    if (prev_odt ^ odt_in) begin
                        ...
                        if ((in_power_down && low_power && (active_bank == 0)) || ($time - tm_slow_exit_pd < TXPDLL) || (ck_cntr - ck_slow_exit_pd < TXPDLL_TCK)) begin
                            odt_state = odt_in;
                            if (DEBUG && odt_en) $display ("%m: at time %t INFO: Async On Die Termination Rtt_NOM = %d Ohm", $time, {32{odt_state}} & get_rtt_nom_func(odt_rtt_nom));
                            if (odt_state) begin
                ...
                if (odt_pipeline[0]) begin
                    odt_state = ~odt_state;
                    if (DEBUG && odt_en) $display ("%m: at time %t INFO: Sync On Die Termination Rtt_NOM = %d Ohm", $time, {32{odt_state}} & get_rtt_nom_func(odt_rtt_nom));
                ...
                if (odt_cntr > 0) begin
                    if ((get_rtt_nom_func(odt_rtt_nom) > 0) && odt_state) begin
                ...
                if (dyn_odt_en && ( odt_state || feature_odt_hi) ) begin
                    if (DEBUG && (dyn_odt_state ^ dyn_odt_pipeline[0]))
                        $display ("%m: at time %t INFO: Sync On Die Termination Rtt_WR = %d Ohm", $time, {32{dyn_odt_pipeline[0]}} & get_rtt_wr_func(odt_rtt_wr));
